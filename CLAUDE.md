...
- **State**: Jotai atoms in `lib/state/atoms.ts`. Live match (`/live`) uses `liveMatchAtom` (`atomWithStorage`) — client-persisted score with optional scorers, "Finalizar" calls `createMatch`. Team builder selection + last-minute guests are also Jotai/localStorage.
