...
- **[Reanimated](https://docs.swmansion.com/react-native-reanimated/)** for animation, with **[Gesture Handler](https://docs.swmansion.com/react-native-gesture-handler/)** under it
- **[Skia](https://shopify.github.io/react-native-skia/)** (`@shopify/react-native-skia`) for canvas drawing — see [Skia on web](#skia-on-web)
...
### Rebuild only when the native shell changes

Almost everything here is JavaScript, and with [EAS Update](https://docs.expo.dev/eas-update/introduction/) those changes reach an installed build over the air. The `channel` on `preview` and `production` is what updates target.

The native app only needs rebuilding after:

- installing a library with native code,
- changing `app.json`,
...
Native gets Skia for free. Web does not: the renderer is CanvasKit, a 7.7 MB WebAssembly binary that has to be served as a static asset and fetched at runtime.

That file is **not in the repo**. `setup-skia-web public` runs as the app's `postinstall`, so `npm install` from anywhere in the monorepo drops a fresh `public/canvaskit.wasm` next to a matching `@shopify/react-native-skia`, and `.gitignore` keeps it out of git. Nothing to run by hand — but if the web build complains that CanvasKit is missing, `npm install` is the fix.

Having the file served is only half of it. Nothing on web can render a `<Canvas>` until CanvasKit has loaded, so a screen that draws with Skia has to be reached through `WithSkiaWeb`, which loads the binary and then the component:
...
<WithSkiaWeb
  ...
  getComponent={() => import("@/components/pitch-canvas")}
/>;
```

Note the module being imported there needs a **default export** — `getComponent` is typed as `Promise<{ default: ComponentType }>`, so the named exports every other component in this app uses will not typecheck. Skia components are the one exception to that convention.

Keep that boundary per screen rather than hoisting it to the root layout: the 7.7 MB download would otherwise sit in front of the first paint of every web visit, including the screens that draw nothing.

`react-native-skia` also ships ~712 MB of prebuilt native binaries that its own `postinstall` copies into place, which the root `allowScripts` has to permit — it is listed there by exact version, so bumping Skia means updating that entry or the next iOS build will fail on missing `libs/`.
